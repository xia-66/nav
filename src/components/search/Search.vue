<template>
  <div :style="{ top: top, borderRadius: store.$state.user.config.searchBorderRadius + 'px' }"
    class="home-search xl:w-px-600 lg:w-px-400 sm:w-px-360 h-px-40">
    <!-- 搜索引擎菜单 -->
    <ul class="menu">
      <li v-for="(item, index) in searchs" :key="index" :data-url="item.url" :data-keyword="item.key"
        @click="selectEngine(index)" class="xl:text-base" :class="{ active: activeSearchIndex === index }">
        {{ item.name }}
      </li>
    </ul>
    <form @submit.prevent="doSearch">
      ...
      <!-- 搜索输入框 -->
      <div class="center">
        <el-input ref="searchInput" v-model="searchText" @input="handleIdea" @focus="handleFocus" @blur="handleBlur"
          :autofocus="true" :placeholder="isSearchInputFocus ? '搜索一下...' : ''" clearable>
        </el-input>
      </div>
      <div class="right">
      ...
      <ul v-if="showIdeas" class="idea gross-glass shadow animate__animated animate__fadeIn">
        <li v-for="(item, index) in ideas" :key="`${item}-${index}`" :class="{ active: index === activeIdeaIndex }"
          @click="doSearchByHover(item, index)">
          <!-- 站内搜索 -->
...
const doSearch = () => {
  if (searchText.value) {
    ...
    if (instationEngine()) {
      // 站内搜索网站链接跳转
      let instationIdeaSite = ideas.value[activeIdeaIndex.value];
      if (instationIdeaSite) url = instationIdeaSite.url;
    } else {
      // 外网搜索跳转
      let params = {};
      params[currentSearch.value.key] = searchText.value;
      if (Object.keys(currentSearch.value.params).length != 0) {
...
// 查询通过hover
const doSearchByHover = (value, index) => {
  activeIdeaIndex.value = index;
  searchText.value = instationEngine() ? value.name : value;
  doSearch();
}
const showIdeas = computed(() => {
  return searchText.value && ideas.value.length !== 0 ? true : false;
})

const showPlaceholder = computed(() => {
  return !searchText.value && !isSearchInputFocus.value;
})// 站内支持搜索网络链接，直接跳转网站
const instationEngine = computed(() => {
  let engine = currentSearch.value;
...
watch(
  ...
  {
    handler (newV, oldV) {
      currentSearch.value = searchs[newV];
    },
    deep: true,
    immediate: true,
  },
